...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size=3, bias=True, in_c=3):
...
class HINet(nn.Module):
    ...
    def __init__(self, in_ch=3, wf=64, depth=5, relu_slope=0.2, hin_position_left=0, hin_position_right=4):
        super(HINet, self).__init__()
        self.depth = depth
        self.down_path_1 = nn.ModuleList()
        self.down_path_2 = nn.ModuleList()
        self.conv_01 = nn.Conv2d(in_ch, wf, 3, 1, 1)
        self.conv_02 = nn.Conv2d(in_ch, wf, 3, 1, 1)
        ...
        for i in reversed(range(depth - 1)):
            ...
            prev_channels = (2**i)*wf
        self.sam12 = SAM(prev_channels, in_c=in_ch)
        self.cat12 = nn.Conv2d(prev_channels*2, prev_channels, 1, 1, 0)

        self.last = conv3x3(prev_channels, in_ch, bias=True)
