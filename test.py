...
if __name__ == '__main__':
	...
	parser.add_argument('--folder_name', type=str, default='08-26')
	args = parser.parse_args()
	log_root = f'/home/coded-deblur/logs/{args.folder_name}'
	...
	for k, date in enumerate(folders):
		date_root = f'{log_root}/{date}'

		date_models = sorted([f for f in os.listdir(date_root) if '.csv' not in f])
		for j, date_model in enumerate(date_models):

			sub_root = f'{date_root}/{date_model}'
			...
			for i, exp in enumerate(experiments):
				...
				file_name = f'{date_root}/mean_results.csv'
				# if file does not exist write header
				if not os.path.isfile(file_name):
	...
	plt.savefig(f'{log_root}/mixed_noise.png', transparent=True, )
